...
class fil_Transfer_skeleton(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, mod='bpsk'):
        ...
        self.connect((self.blocks_throttle2_0, 0), (self.pfb_clock_sync, 0))      # replaces RRC + symbol_sync
        self.connect((self.pfb_clock_sync, 0), self.digital_costas_loop_cc_0)
        self.connect(self.digital_costas_loop_cc_0, self.digital_constellation_decoder_cb_0)
        self.connect((self.digital_costas_loop_cc_0,0), (self.constellation_plot, 0))
        self.connect((self.digital_constellation_decoder_cb_0, 0), self.digital_diff_decoder_bb_0)
        self.connect(self.digital_diff_decoder_bb_0, (self.unpack, 0))
        self.connect((self.unpack, 0), self.destination)
